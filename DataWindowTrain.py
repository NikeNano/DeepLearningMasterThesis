...
# Class for import of the data to the "FFNN.py" model. 
# This model is uses a slidingwindow. The window size can be change but is default set to 20. 
class DataWindowTrain():
	def __init__(self,pathLabel="traningLabels.pickle", pathData="traningData.pickle",windowSize=20):
	...
	def importData(self): # imports the data
		patient_data=[]
		patient_labels=[]
		patient_seq_length=[]
		print("test")
	...
	def splitData(self): # split the data in traning and test set
		# split the data in test and training set
		split=int(np.rint(len(self.window_data)*0.8))
		self.window_data, self.window_label = shuffle(self.window_data,self.window_label)
		self.test_data=self.window_data[split+self.windowSize:]
		self.test_label=self.window_label[split+self.windowSize:]
		self.data_out=self.window_data[:split]
		self.label_out=self.window_label[:split]

	def nextWindowBatch(self, batch_size): # feed sthe next batch. Batch size set as paramters and could be changed. 
	...
	def testWindowBatch(self): # Returns the test data
		return (self.test_data, self.test_label)

	def oneHotEncoded4Labels(self): # One hot encode the data when four labels are used. This was not done in the end
		temp_list_label=[]
	...
	def oneHotEncoded2Labels(self): # One hot encode the data when two labels are used. This was done in the end. 
		temp_list_label=[]
		#embed()
		for label in self.window_label:
			if label==0:
	...
	def patientReferences(self): # This collects the patients ID:s from the data. This is used when the data is split. 
		temp_list=[]
	...
	def countLabels4Labels(self): # Count the lables when 4 labels are used. Not done in the end
	...
	def countLabels2Labels(self): # Counts the distribution of the data when two labels are used. 
		# This is a function to return the distribution of the sequences! Good to know
		# Have how ever been throuing out a lot of data! This is pretty sad. 
		count0=count1=count2=count3=count4=0
		labels=self.window_label
